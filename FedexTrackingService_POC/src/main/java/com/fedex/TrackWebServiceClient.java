...
public class TrackWebServiceClient {
	...
	public WriteEventLogResponse1 trackWebServiceClient(WriteEventLog trackingRequest){
		//
		...
		try {
			...
			WriteEventLogResponse customreply = new WriteEventLogResponse();
			//customreply.getCompletedTrackDetails().add((com.fedex.ws.track.v12.CompletedTrackDetail) Arrays.asList(reply.getCompletedTrackDetails()));
			com.fedex.ws.track.v12.CompletedTrackDetail completedTrackDetail = new com.fedex.ws.track.v12.CompletedTrackDetail();
			completedTrackDetail.setDuplicateWaybill(reply.getCompletedTrackDetails()[0].getDuplicateWaybill());
			//completedTrackDetail.getHighestSeverity.(reply.getCompletedTrackDetails()[0].getHighestSeverity().getValue());
			completedTrackDetail.setPagingToken(reply.getCompletedTrackDetails()[0].getPagingToken());
			completedTrackDetail.setTrackDetailsCount(reply.getCompletedTrackDetails()[0].getTrackDetailsCount());
			...
			//customreply.getNotifications().add((com.fedex.ws.track.v12.Notification) Arrays.asList(reply.getNotifications()));
			/*if(reply.getTransactionDetail().getCustomerTransactionId()!=null){
				customreply.getTransactionDetail().setCustomerTransactionId(reply.getTransactionDetail().getCustomerTransactionId());
			}*/
			com.fedex.ws.track.v12.Localization localization = new com.fedex.ws.track.v12.Localization();
			if(reply.getTransactionDetail().getLocalization()!=null){
			localization.setLanguageCode(reply.getTransactionDetail().getLocalization().getLanguageCode());
			localization.setLocaleCode(reply.getTransactionDetail().getLocalization().getLocaleCode());
			customreply.getTransactionDetail().setLocalization(localization);}
			com.fedex.ws.track.v12.VersionId version = new com.fedex.ws.track.v12.VersionId();
			version.setIntermediate(reply.getVersion().getIntermediate());
			version.setMajor(reply.getVersion().getMajor());
			version.setMinor(reply.getVersion().getMinor());
			version.setServiceId(reply.getVersion().getServiceId());
			customreply.setVersion(version);
			com.fedex.ws.track.v12.Notification notifications = new com.fedex.ws.track.v12.Notification();
			notifications.setCode(reply.getNotifications()[0].getCode());
			notifications.setLocalizedMessage(reply.getNotifications()[0].getLocalizedMessage());
			notifications.setMessage(reply.getNotifications()[0].getMessage());
			//notifications.setSeverity(reply.getNotifications()[0].getSeverity());
			notifications.setSource(reply.getNotifications()[0].getSource());
			customreply.getNotifications().add(notifications);
			//return customreply;
			WriteEventLogResponse1 response1 = new WriteEventLogResponse1();
			response1.setCompletedTrackDetails(reply.getCompletedTrackDetails());
			response1.setHighestSeverity(reply.getHighestSeverity());
			response1.setNotifications(reply.getNotifications());
			response1.setTransactionDetail(reply.getTransactionDetail());
			response1.setVersion(reply.getVersion());
	...
	private static ClientDetail createClientDetail() {
        ...
        if (accountNumber == null) {
        	accountNumber = "510087666"; // Replace "XXX" with clients account number
        }
        if (meterNumber == null) {
        	meterNumber = "118775182"; // Replace "XXX" with clients meter number
        }
	...
	private static WebAuthenticationDetail createWebAuthenticationDetail() {
        ...
        //
        if (key == null) {
        	key = "xqzzARfm7rvu1Q7k"; // Replace "XXX" with clients key
        }
        if (password == null) {
        	password = "1OWDGau3rKU4xvDrg7eC7VC2j"; // Replace "XXX" with clients password
        }
